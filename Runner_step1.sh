...
#    ((j=j%4)); ((j++==0)) && wait
    java -Xmx7000m -Xms7000m -jar runnable/LightWeightCGs-1.0-SNAPSHOT-with-dependencies.jar --wholeProgram "$line" $folder/$coord/opal > $folder/$coord/opal/log 2>&1
    java -Xmx7000m -Xms7000m -jar runnable/LightWeightCGs-1.0-SNAPSHOT-with-dependencies.jar --wholeProgram "$line" $folder/$coord/wala > $folder/$coord/wala/log 2>&1
