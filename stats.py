...
print("####### Edge Plot ####### \n ")
fig, (ax1) = plt.subplots(nrows=1, ncols=2, sharey=True)
mergeEdge = pd.to_numeric(overall_fair[(overall_fair['mergeEdges']) !=0 & (overall_fair['mergeEdges'] != -1)], downcast="float")
opalEdge = pd.to_numeric(overall_fair[(overall_fair['opalEdges'] !=0) & (overall_fair['opalEdges'] != -1)].astype(float), downcast="float")
